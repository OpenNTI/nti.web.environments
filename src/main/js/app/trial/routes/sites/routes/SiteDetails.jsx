...
import { Page, AuthRouter, Text, Link } from 'internal/common';
...
export default function SiteDetails({ siteId, location }) {
	...
	const siteLoading =
		isPending(siteResolver) || customerLoading || !siteResolver;
	...
	const showLoading =
		siteLoading ||
		site.isPending ||
		(site.wasPending && !loadAnimationFinished);

	const currentState = location.hash;
	const state = showLoading
		? '#loading'
		: site.isSuccess
		? '#success'
		: '#failure';
			...
			<Page.Content className={cx('section')} fullscreen>
				{siteError && (
					<div className={cx('missing-site')}>
						<Text.Heading>{t('notfound.heading')}</Text.Heading>
						<Text.Paragraph>
				...
				{state !== currentState ? (
					<Redirect to={`/sites/${siteId}/${state}`} noThrow />
				) : null}
			</Page.Content>
		</Page>
	);
