...
export function validateDomain (domain) {
	return !(
		...
		domain.length > MaxLength	    //Has more than 63 characters
	);
}

export function getDomainError (domain) {
	if (domain.match(/[^a-z0-9-]/g)) { return new Error(t('validCharacters')); }
	if (domain.match(/^[^a-z]/)) { return new Error(t('leadingLetter')); }
	if (domain.match(/[^a-z0-9]$/))	{ return new Error(t('trailingValid')); }
	if (domain.length > MaxLength) { return new Error(t('tooLong')); }
	if (domain.length < MinLength) { return new Error(t('tooShort')); }
