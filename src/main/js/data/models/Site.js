...
export default class Site {
	...
	static hasCached(id) {
		return Site.Cache.has(id);
	}
	static getCached(id) {
		return Site.Cache.get(id);
	}
	...
	getLink(rel) {
		return getLink(this.#data.Links, rel);
	}
	...
	get domain() {
		return (this.#data.dns_names || [])[0];
	}
	get continueLink() {
		return this.getLink('setup.continue');
	}
	get pollLink() {
		return this.getLink('setup.check-pending');
	}
	...
	get setupState() {
		return this.#data['setup_state'];
	}
	get state() {
		return this.setupState?.MimeType;
	}
	get isPending() {
		return (
			(!this.state && this.status === 'PENDING') ||
			this.state === SiteStates.Pending
		);
	}
	get isSuccess() {
		return this.state === SiteStates.Success;
	}
	get isFailure() {
		return this.state === SiteStates.Failure;
	}
