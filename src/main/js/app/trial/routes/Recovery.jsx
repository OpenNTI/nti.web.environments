	...
	return (
		<Page title={t('title')}>
			<Page.Content className={cx('recover', {saving})}>
				...
				{sentTo && (<Text.Paragraph centered>{t('sentTo')}</Text.Paragraph>)}
				{!sentTo && (<Text.Paragraph centered>{t('message')}</Text.Paragraph>)}
				{sentTo && (
					<div className={cx('sent-to')}>
						<Text.Paragraph centered><strong>{sentTo}</strong> <a href="#" onClick={clearSentTo}>{t('edit')}</a></Text.Paragraph>
					</div>
				)}
				{!sentTo && (
