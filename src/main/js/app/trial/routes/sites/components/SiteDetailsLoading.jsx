...
function embedMeetingScript() {
	if (!global.document || embedMeetingScript.script) {
		return;
	}

	const script = Object.assign(global.document.createElement('script'), {
		async: true,
		defer: true,
		charset: 'utf-8',
...
export default function SiteDetailsLoading({ site, onFinished }) {
	...
	const doSkipForm = () =>
		isPending(finished) ? setSkipForm(true) : onFinished();

	return (
		<section
			className={cx('site-loading-details', { 'show-form': doShowForm })}
		>
			<LoadingSVG
				progress={progress}
			...
			<div className={cx('form')}>
				<Text.Heading localized className={cx('heading')}>
					{t('whileYouWait')}
				</Text.Heading>
				<div
					className={'frame'}
					ref={maybeLoadScript}
					{...rawContent(MeetingFormEmbed)}
				/>
			</div>
				...
				<TransitionGroup className={cx('messages')}>
					...
					>
						<Text.Paragraph className={cx('message')}>
							We are getting everything just right. <br /> This
							may take a moment...
						</Text.Paragraph>
					</CSSTransition>
				</TransitionGroup>
			</div>
			<Text.Base className={cx('continue')} onClick={doSkipForm}>
				{t('continue')}
			</Text.Base>
